...
func Test_Repo_Category_Get(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			area := sqlmock.NewRows([]string{"id", "name", "sub_category", "is_active", "updated_at"}).AddRow(test.expectedReturn[0].Id, test.expectedReturn[0].Name, test.expectedReturn[0].SubCategory, test.expectedReturn[0].IsActive, test.expectedReturn[0].UpdateAt)
