...
func (repo *repository) GetUserDetail(request string) (entity.User, error) {
	...
	error := repo.db.Raw("SELECT users.*, JSON_AGG(JSON_BUILD_OBJECT('id', roles.id, 'name', roles.name)) AS roles, JSON_AGG(DISTINCT user_has_area.area) AS area_code, JSON_AGG(DISTINCT user_has_region.regional) AS regional, JSON_AGG(DISTINCT user_has_grapari.grapari_id) AS grapari_id FROM users LEFT OUTER JOIN roles ON (roles.id = users.rule_id) LEFT OUTER JOIN user_has_area ON (users.username = user_has_area.username) LEFT OUTER JOIN user_has_region ON (users.username = user_has_region.username) LEFT OUTER JOIN user_has_grapari ON (users.username = user_has_grapari.username) WHERE users.username = ? GROUP BY users.id ORDER BY users.name ASC", request).Find(&user).Error
...
func (repo *repository) GetUserGroupByRole() ([]model.GetUserGroupByRole, error) {
	...
	error := repo.db.Raw("SELECT roles.id, roles.name AS label, JSON_AGG(JSON_BUILD_OBJECT('label', users.name, 'value', users.username)) AS options FROM ticketing_user_has_role INNER JOIN roles ON (roles.id = ticketing_user_has_role.id_role) INNER JOIN users ON (users.id = ticketing_user_has_role.id_user) WHERE users.status = 'Active'  GROUP BY roles.name, roles.id ORDER BY roles.name ASC").Find(&user).Error
