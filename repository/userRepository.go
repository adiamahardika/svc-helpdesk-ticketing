...
func (repo *repository) CountUser(request model.GetUserRequest) (int, error) {
	...
	error := repo.db.Raw("SELECT COUNT(*) as total_data FROM users WHERE users.name LIKE @Search OR users.username LIKE @Search OR users.email LIKE @Search", model.GetUserRequest{
		Search: "%" + request.Search + "%",
