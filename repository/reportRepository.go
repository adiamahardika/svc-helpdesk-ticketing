...
func (repo *repository) GetReport(request model.GetReportRequest) ([]entity.Ticket, error) {
	...
	if len(request.Category) == 0 {
		query = "SELECT ticket.*, category.name AS kategori FROM ticket LEFT OUTER JOIN category ON (ticket.kategori = CAST(category.id AS varchar(10))) WHERE prioritas IN @Priority AND status IN @Status AND assigned_to LIKE @AssignedTo AND username_pembuat LIKE @UsernamePembuat AND tgl_dibuat >= @StartDate AND tgl_dibuat <= @EndDate ORDER BY tgl_dibuat ASC"
	} else {
		query = "SELECT ticket.*, category.name AS kategori FROM ticket LEFT OUTER JOIN category ON (ticket.kategori = CAST(category.id AS varchar(10))) WHERE prioritas IN @Priority AND status IN @Status AND assigned_to LIKE @AssignedTo AND username_pembuat LIKE @UsernamePembuat AND kategori IN @Category AND tgl_dibuat >= @StartDate AND tgl_dibuat <= @EndDate ORDER BY tgl_dibuat ASC"
	}
