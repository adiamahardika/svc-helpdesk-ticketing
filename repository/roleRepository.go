...
func (repo *repository) GetDetailRole(request model.GetRoleRequest) ([]entity.Role, error) {
	...
	error := repo.db.Raw("SELECT role.*, JSON_AGG(JSON_BUILD_OBJECT('id', permission.id, 'name', permission.name, 'code', permission.permission_code)) AS list_permission FROM role_has_permission INNER JOIN role ON (role.id = role_has_permission.id_role) INNER JOIN permission ON (role_has_permission.id_permission = permission.id) INNER JOIN user_has_role ON (user_has_role.id_role = role.id) WHERE user_has_role.id_user = @IdUser GROUP BY role_has_permission.id_role, role.id ORDER BY role.name ASC", request).Find(&role).Error
