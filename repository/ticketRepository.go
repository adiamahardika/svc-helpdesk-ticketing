...
func (repo *repository) UpdateTicket(request *model.UpdateTicketRequest) ([]entity.Ticket, error) {
	...
	error := repo.db.Raw("UPDATE ticket SET assigned_to =  @AssignedTo, category = @Category, sub_category = @SubCategory,  prioritas = @Prioritas, status = @Status, updated_by = @UpdatedBy, tgl_diperbarui = @TglDiperbarui, assigning_time = @AssigningTime, assigning_by = @AssigningBy, visit_status = @VisitStatus WHERE ticket_code = @TicketCode RETURNING ticket.*", request).Find(&ticket).Error
