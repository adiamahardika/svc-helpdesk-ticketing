...
func (repo *repository) GetDetailTicket(ticket_code string) (entity.Ticket, error) {
	...
	error := repo.db.Raw("SELECT ticket.*, ms_area.area_name, ms_grapari.name AS grapari_name FROM ticket LEFT OUTER JOIN ms_area ON (ticket.area_code = ms_area.area_code) LEFT OUTER JOIN ms_grapari ON (ticket.grapari_id = ms_grapari.grapari_id) WHERE ticket_code = ?", ticket_code).Find(&ticket).Error
