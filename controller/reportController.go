...
func (controller *reportContoller) GetCountReportByStatus(context *gin.Context) {

	var request *model.GetCountReportByStatusRequest

	error := context.ShouldBindJSON(&request)
	description := []string{}
	http_status := http.StatusOK
	var status *model.StandardResponse
	var ticket []model.GetCountReportByStatusResponse
	...
	if error != nil {
		...
		ticket, error = controller.reportService.GetCountReportByStatus(request)
